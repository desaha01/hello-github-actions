...
class MCPClient:
    ...
    async def get_available_tools(self) -> List[Any]:
        ...
        try:
            async with client_session(self.mcp._mcp_server) as client:
                ...
                logger.debug(f"Received tools from MCP server: {tools_result}")
                return tools_result.tools
        except Exception as e:
            logger.error(f"Failed to get available tools from MCP server: {e}")
            raise RuntimeError("Failed to get available tools from MCP server")
    ...
    async def call_tool(self, tool_name: str, arguments: dict) -> Any:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to call tool '{tool_name}' with arguments: {arguments}")
